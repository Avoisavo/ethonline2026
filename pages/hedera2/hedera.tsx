...
function CreateAccount() {
  ...
  const handleSubmit = async () => {
    ...
    setResult(null);
    try {
      const data = await callApi("/api/hedera2/hedera/create-account", {
...
function CreateToken() {
  ...
  const handleSubmit = async () => {
    ...
    setResult(null);
    try {
      const data = await callApi("/api/hedera2/hedera/create-token", {
        tokenName: name,
        tokenSymbol: symbol,
...
function CreateTopic() {
  ...
  const handleCreateTopic = async () => {
    ...
    setResult(null);
    try {
      const data = await callApi("/api/hedera2/hedera/create-topic", { memo });
      setResult(data);
  ...
  const handleSubmitMessage = async () => {
    ...
    setMsgResult(null);
    try {
      const data = await callApi("/api/hedera2/hedera/submit-message", {
        topicId,
        message,
      });
      setMsgResult(data);
...
function ScheduleTransaction() {
  ...
  const handleSubmit = async () => {
    ...
    setResult(null);
    try {
      const data = await callApi("/api/hedera2/hedera/schedule-transaction", {
...
function HCS20Points() {
  ...
  const call = async (action: string, body: Record<string, unknown>) => {
    ...
    setResult(null);
    try {
      const data = await callApi("/api/hedera2/hcs/hcs20", { action, ...body });
      setResult(data);
...
function HCS2Registry() {
  ...
  const call = async (action: string, body: Record<string, unknown>) => {
    ...
    setResult(null);
    try {
      const data = await callApi("/api/hedera2/hcs/hcs2", { action, ...body });
      setResult(data);
...
function HCS11Profile() {
  ...
  const call = async (action: string, body: Record<string, unknown>) => {
    ...
    setResult(null);
    try {
      const data = await callApi("/api/hedera2/hcs/hcs11", { action, ...body });
      setResult(data);
...
function HCS16Flora() {
  ...
  const call = async (action: string, body: Record<string, unknown>) => {
    ...
    setResult(null);
    try {
      const data = await callApi("/api/hedera2/hcs/hcs16", { action, ...body });
      setResult(data);
      if (action === "create") {
